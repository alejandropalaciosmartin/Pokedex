...
const urlPokeApi = `https://pokeapi.co/api/v2/pokemon/?offset=${j}&limit=${j}`; //Coger los siguientes 20 pokemons
...
async function take(url){ 
    ...
    try{
        ...
        if(!res.ok) throw{status:res.status,statusText: statusText} //Si la respuesta de la API no (!) es ok(es la respuesta que da el
         ...
         //se usa for porque es bloqueante y podemos ir cogiendo y analizando cada paso, con 
            ...
            try{
                let res = await fetch(json.url), //cogemos de la api el json creado al principio, de ese json de la url
...
document.addEventListener("DOMContentLoaded",e=>take(urlPokeApi)); //Donde se va a cargar todo
